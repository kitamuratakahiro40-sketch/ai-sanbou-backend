 ...
 */
async function processNewAudioFile(gcsUri, sliceSec = 600) {
    ...
    // 3) DB登録（jobs, chunks）
    await query(`INSERT INTO jobs(id, source_uri, duration_sec, chunk_sec, total_chunks, status)
     VALUES ($1,$2,$3,$4,$5,'RUNNING')`, [jobId, gcsUri, durationSec, sliceSec, plan.total]);
    for (const it of plan.items) {
        await query(`INSERT INTO chunks(job_id, idx, start_sec, end_sec, status)
       VALUES ($1,$2,$3,$4,'PENDING')`, [jobId, it.idx, it.start, it.end]);
    }
    // 4) Cloud Tasks へ投入
    for (const it of plan.items) {
    ...
    return { jobId, durationSec, totalChunks: plan.total, status: 'RUNNING' };
}
// ========== Eventarc（GCS最終化）受入口：POST / ==========
// Cloud Storage 直接のEventarc（CloudEvents形式）と、Pub/Sub転送の両方に耐える柔軟実装。
function extractGcsUriFromEventarc(req) {
    try {
...
app.get('/jobs/:id', async (req, res) => {
    ...
    try {
        const { id } = req.params;
        const j = await query(`SELECT id, status, status_detail, total_chunks, duration_sec, chunk_sec, created_at, updated_at
         FROM jobs WHERE id = $1`, [id]);
        if (!j.rows[0])
            return res.status(404).json({ error: 'not_found' });
        const c = await query(`SELECT status, COUNT(*)::text AS count
...
app.post('/jobs/:jobId/retry', async (req, res) => {
    ...
    try {
        ...
        const rows = await query(`SELECT c.idx, c.start_sec, c.end_sec, j.source_uri AS gcs_uri
         FROM chunks c
         JOIN jobs j ON j.id=c.job_id
        WHERE c.job_id=$1
          AND c.status IN ('FAILED','PENDING')
        ORDER BY c.idx ASC`, [jobId]);
        if (rows.rowCount === 0) {

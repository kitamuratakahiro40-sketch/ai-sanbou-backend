...
"SELECT id, index, payload_json FROM chunks WHERE job_id=$1 AND status='FAILED' ORDER BY index ASC",
[jobId]
);
...
// PENDING に戻す
await client.query(
"UPDATE chunks SET status='PENDING' WHERE job_id=$1 AND status='FAILED'",
[jobId]
);
...
// status_detail の追記
await client.query(
"UPDATE jobs SET status='RUNNING', status_detail=COALESCE(status_detail,'') || $2 WHERE id=$1",

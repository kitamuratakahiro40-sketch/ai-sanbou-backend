...
const DEFAULT_LOCATION = process.env.VERTEX_LOCATION || process.env.LOCATION || "us-central1";
...
async function saveTranscriptToGCS(jobId, text) {
        ...
        contents: [
            {
                ...
                parts: [
                    {
                        text: "You are a professional transcriber. Output only transcript. Keep original language.",
                    },
...
function resolveKey(idx, chunkId) {
    if (typeof idx === "number")
        return { mode: "idx", key: idx };
    if (typeof chunkId === "string" && chunkId.length > 0)
        return { mode: "chunkId", key: chunkId };
    throw new Error("idx か chunkId のいずれかが必須です");
}
async function markChunkRunning(jobId, idKey, retryCount) {
    if (idKey.mode === "idx") {
        await query(`UPDATE chunks
         SET status='RUNNING', retry_count=$1, updated_at=now()
       WHERE job_id=$2 AND idx=$3`, [retryCount, jobId, idKey.key]);
    }
    else {
        await query(`UPDATE chunks
         SET status='RUNNING', retry_count=$1, updated_at=now()
       WHERE job_id=$2 AND id=$3`, [retryCount, jobId, idKey.key]);
    }
}
async function markChunkDone(jobId, idKey, text) {
    if (idKey.mode === "idx") {
        await query(`UPDATE chunks
         SET status='DONE', text=$1, updated_at=now()
       WHERE job_id=$2 AND idx=$3`, [text, jobId, idKey.key]);
    }
    else {
        await query(`UPDATE chunks
         SET status='DONE', text=$1, updated_at=now()
       WHERE job_id=$2 AND id=$3`, [text, jobId, idKey.key]);
    }
}
async function markChunkFailed(jobId, idKey, errMsg) {
    if (idKey.mode === "idx") {
        await query(`UPDATE chunks
         SET status='FAILED', error_msg=$1, updated_at=now()
       WHERE job_id=$2 AND idx=$3`, [errMsg, jobId, idKey.key]);
    }
    else {
        await query(`UPDATE chunks
         SET status='FAILED', error_msg=$1, updated_at=now()
       WHERE job_id=$2 AND id=$3`, [errMsg, jobId, idKey.key]);
    }
}
// すべて完了したら transcript を確定し、GCS に保存、jobs を COMPLETED へ
async function finalizeIfAllDone(jobId) {
    // まだ終わっていないチャンクがあるなら何もしない
    const pend = await query(`SELECT count(*) FROM chunks WHERE job_id=$1 AND status <> 'DONE'`, [jobId]);
    if (Number(pend.rows[0].count) !== 0)
        return;
    // 全文を連結
    const parts = await query(`SELECT idx, COALESCE(text,'') AS text
       FROM chunks
      WHERE job_id=$1
      ORDER BY idx ASC`, [jobId]);
...
const transcribeHandler = async (req, res) => {
    const b = (req.body || {});
    const jobId = b.jobId;
    const gcsUri = b.gcsUri;
    // idx / chunkId どちらでもOK
    const idx = typeof b.idx === "number"
        ? b.idx
        : typeof b.chunkIndex === "number"
            ? b.chunkIndex
            : undefined;
    const chunkId = typeof b.chunkId === "string" ? b.chunkId : undefined;
    // 時刻正規化（ms優先、なければsec→ms）
    const startMs = typeof b.startMs === "number"
        ? b.startMs
        : typeof b.startSec === "number"
            ? Math.round(b.startSec * 1000)
            : undefined;
    const endMs = typeof b.endMs === "number"
        ? b.endMs

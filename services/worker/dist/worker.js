...
async function transcribeWav(wavPath) {
    ...
    const text = resp.response?.candidates?.[0]?.content?.parts
        ...
        .trim() || "";
    if (!text)
        throw new Error("Empty transcript from model");
    return text;
}
/**
 * チャンク完了毎に呼び、ジョブ完了なら transcripts 生成 & GCS へ out/<jobId>.txt 保存
 */
async function maybeFinalize(jobId) {
    ...
    try {
       ...
       WHERE job_id = $1`, [jobId]);
        ...
        if (Number(failed) > 0) {
            await client.query(`UPDATE jobs SET status='FAILED', status_detail=$2, updated_at=NOW() WHERE id=$1`, [jobId, "One or more chunks FAILED"]);
            return;
        }
        if (Number(pending) > 0 || Number(running) > 0)
            return;
        const qTexts = await client.query(`SELECT idx, COALESCE(text,'') AS text
         FROM chunks
        WHERE job_id=$1
        ORDER BY idx`, [jobId]);
        const full = qTexts.rows.map((r) => r.text).join("\n").trim();
        await client.query(`INSERT INTO transcripts(job_id, text, created_at, updated_at)
       VALUES($1, $2, NOW(), NOW())
       ON CONFLICT (job_id)
       DO UPDATE SET text=EXCLUDED.text, updated_at=NOW()`, [jobId, full]);
        await client.query(`UPDATE jobs SET status='COMPLETED', status_detail=$2, updated_at=NOW() WHERE id=$1`, [jobId, `Transcription assembled (${qTexts.rows.length} chunks)`]);
        if (!BUCKET_NAME) {
...
app.post("/tasks/transcribe", async (req, res) => {
    ...
    const retryHeader = req.header("Cloud-Tasks-Task-Retry-Count") ||
    ...
    try {
        ...
        await client.query(`UPDATE chunks
         SET status='RUNNING', retry_count=$1, updated_at=NOW()
       WHERE job_id=$2 AND idx=$3`, [retryCount, jobId, idx]);
        // 作業DIR
        ...
        const sSec = typeof startMs === "number" ? startMs / 1000 : 0;
        const eSec = (typeof endMs === "number" ? endMs / 1000 : sSec + 60);
        // 念のため、end > start を保証
        const safeEnd = eSec > sSec ? eSec : sSec + 1;
       ...
       WHERE job_id=$5 AND idx=$6`, [
        ...
        try {
           ...
           SET status='FAILED',
               ...
               retry_count=$2,
               updated_at=NOW()
         WHERE job_id=$3 AND idx=$4`, [String(err?.message || err), retryCount, jobId, idx]);
            await client.query(`UPDATE jobs
           SET status='RUNNING',
               status_detail=$2,
               updated_at=NOW()
         WHERE id=$1`, [jobId, "Chunk failed (retry or /retry)"]);
        }
        catch (e2) {
        ...
        // 片付け
        try {
            await fs.rm(path.join(TMP_DIR, jobId, String(idx)), { recursive: true, force: true });
        }
        catch { }
    }
});
